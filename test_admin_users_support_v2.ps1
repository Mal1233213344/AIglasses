...
$updateBody = @{ status = 'closed' } | ConvertTo-Json
$updateResp = Invoke-RestMethod -Method Patch -Uri "$base/admin/v1/support/tickets/$ticketId/status" -ContentType 'application/json' -Headers @{ Authorization = "Bearer $adminToken" } -Body $updateBody
